...
"use strict";require("babel-polyfill");var _api=require("api");var _api2=_interopRequireDefault(_api);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}'use strict';var monitor=new _api2.default('./api');function update_plots(){var tbody=$('#plots > tbody');tbody.empty();var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{var _loop=function _loop(){var plot=_step.value;var tr=$('<tr></tr>').appendTo(tbody);$('<td></td>').appendTo(tr).append($('<a></a>').text(plot.comment!=''?plot.comment:'<no comment>').attr('href','./plot.html?'+plot.id));$('<td></td>').appendTo(tr).attr('class','col-md-1').append($('<button></button>').text('Delete').attr('type','button').attr('class','btn btn-danger').on('click',function(){return monitor.plots.remove(plot).then(update_plots);}));};for(var _iterator=monitor.plots[Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){_loop();}}catch(err){_didIteratorError=true;_iteratorError=err;}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return();}}finally{if(_didIteratorError){throw _iteratorError;}}}}function update_logs(){var tbody=$('#logs > tbody');tbody.empty();var _iteratorNormalCompletion2=true;var _didIteratorError2=false;var _iteratorError2=undefined;try{var _loop2=function _loop2(){var log=_step2.value;var tr=$('<tr></tr>').appendTo(tbody);$('<td></td>').appendTo(tr).text(log.path);$('<td></td>').appendTo(tr).text(log.comment);$('<td></td>').appendTo(tr).attr('class','col-md-1').append($('<button></button>').text('Delete').attr('type','button').attr('class','btn btn-danger').on('click',function(){return monitor.logs.remove(log).then(update_logs);}));};for(var _iterator2=monitor.logs[Symbol.iterator](),_step2;!(_iteratorNormalCompletion2=(_step2=_iterator2.next()).done);_iteratorNormalCompletion2=true){_loop2();}}catch(err){_didIteratorError2=true;_iteratorError2=err;}finally{try{if(!_iteratorNormalCompletion2&&_iterator2.return){_iterator2.return();}}finally{if(_didIteratorError2){throw _iteratorError2;}}}}function add_plot(){var text=$('#comment');monitor.plots.add({comment:text.val()}).then(function(){update_plots();text.val('');});}$(document).ready(function(){monitor.plots.sync().then(update_plots);monitor.logs.sync().then(update_logs);});
